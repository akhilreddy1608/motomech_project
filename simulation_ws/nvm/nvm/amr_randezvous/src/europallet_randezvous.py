...
class EuropalletRandezVous(object):
    ...
    def __init__(self):
        ...
        #self.restart_pub = rospy.Publisher("/restart", Float64, queue_size=1)
   ...
   # def move_forward2(self,velocity,p,p_goal):      
   #     if p_goal > p:
    #        v = abs(velocity) * (abs(p_goal) - abs(p))        
    #    elif p_goal < p:
     #        v = -abs(velocity) * (abs(p) - abs(p_goal))          
     #   else:
     #       v = 0
    ...
    #def move_forward(self,velocity,p,p_goal):      
     #   if p_goal > p:
     #       v = abs(velocity) * (abs(p_goal) - abs(p))        
     #   elif p_goal < p:
      #       v = -abs(velocity) * (abs(p) - abs(p_goal))          
    ...
    def callback_odom(self,msg):     
        ...
        if self.phase == "1":  
            ...
            print("self.y",self.y," self.yg ",self.yg)  
        ...
        #elif self.phase == "4":
           # rospy.loginfo(self.phase)
           # print("self.x",self.x," self.xg ",self.xg-0.3)
          #  i = (self.yg - self.y) / self.yg     
           # self.cmd_vel.linear.x = -0.5 * math.exp(i) - 0.02
          #  self.cmd_vel_pub.publish(self.cmd_vel) 
            
           # if self.theta < 3 or self.theta > -3:
           #     self.phase = "3"
          #  if self.x > self.xg - 0.5: 
           #     self.stop_moving()
           #     self.phase = "5"
...
"""
    ...
    def get_work_zone(self,delta_x):

            if  delta_x < self.x_inf:
                wz = "L"                
            elif delta_x > self.x_sup:
                wz = "R"  
            else:
                wz = "C"
            rospy.loginfo(wz)
    ...
    def get_delta_pose(self):
        ...
        if self.result.success:
            ...
            q_goal = [q0,q1,q2,q3]
            (roll,pith,yaw) = euler_from_quaternion(q_goal)
            goal_delta_angle = yaw
            return goal_delta_angle, goal_delta_pose.position.x, goal_delta_pose.position.y
        else:
            rospy.logwarn(self.result.output)

    
    def quat_to_angle(self,quaternion):
        (roll,pith,yaw) = euler_from_quaternion(quaternion)
        return yaw
    ...
    def move_turn(self,velocity):
        self.cmd_vel.angular.z = velocity
        self.cmd_vel_pub.publish(self.cmd_vel)  
